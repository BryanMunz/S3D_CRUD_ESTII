...
$pdf = new FPDF($orientation='P', $unit='mm', array(80, 270));
$pdf->AddPage();
...
$usuario = mysqli_query($conexion, "SELECT nombre FROM usuario WHERE idusuario = $id_user");
//$nombreUsuario = mysqli_fetch_assoc($usuario)['nombre'];

// Obtener la venta
$ultimaVentaQuery = "SELECT * FROM ventas ORDER BY id DESC LIMIT 1";
$ultimaVentaResult = mysqli_query($conexion, $ultimaVentaQuery);
$ultimaVenta = mysqli_fetch_assoc($ultimaVentaResult);
$id = $ultimaVenta['id'];
...
if (!empty($ultimaVenta)) {
    ...
    // Obtener el cliente relacionado con la venta
    $clienteQuery = "SELECT * FROM cliente WHERE idcliente = " . $ultimaVenta['id_cliente'];
    $clienteResult = mysqli_query($conexion, $clienteQuery);
    $datosC = mysqli_fetch_assoc($clienteResult);
...
$pago = 0.00;
$cambio = 0.00;
$ventaId = $ultimaVenta['id'];// Obtener el ID de la última venta
$detalleVentaQuery = "SELECT d.*, COALESCE(p.descripcion, d.producto_c) as producto_descripcion, p.codproducto FROM detalle_venta d LEFT JOIN producto p ON d.id_producto = p.codproducto WHERE d.id_venta = $ventaId";
$detalleVentaResult = mysqli_query($conexion, $detalleVentaQuery);

$Y= 68;
while ($row = mysqli_fetch_assoc($detalleVentaResult)) {
    $pdf->SetFont('Arial', '', 7);
...
$CambioQuery = "SELECT * FROM ventas WHERE id = $ventaId";
...
$descuentoVentaQuery = "SELECT d.*, p.codproducto, p.p_venta, CASE WHEN d.id_producto = 0 THEN (d.cantidad * d.precio) ELSE (d.cantidad * p.p_venta) 
    END AS Total FROM detalle_venta d LEFT JOIN producto p ON d.id_producto = p.codproducto WHERE d.id_venta = $ventaId";
