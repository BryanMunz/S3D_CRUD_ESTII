...
if(empty($id_computadora)){
    echo '<script>
            alert("Error, el registro no se encontro");
            window.location = "../historial_computadora.php";
            </script>';
}//end empty
else{
...
$Compus = mysqli_query($conexion, "SELECT * FROM computadoras WHERE id = $id_computadora ORDER BY id DESC LIMIT 1");
$datosCompus = mysqli_fetch_assoc($Compus);


if(is_null($datosCompus)){
    echo '<script>
          alert("Error, el registro no esxiste");
          window.location = "../historial_computadora.php";
          </script>';
  }
